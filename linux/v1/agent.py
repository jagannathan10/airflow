...
TMUX_BIN = "/usr/bin/tmux"  # explicit path to tmux on RHEL
...
async def validate_token(request: Request):
    ...
    if not header or header != AGENT_TOKEN:
        # You can change this to HTTPException if you prefer
...
# -----------------------------------------------------------------------------
# SYNC EXECUTION (no tmux, wait for completion)
# -----------------------------------------------------------------------------

def execute_sync(payload: JobRequest) -> Dict:
    """
    Synchronous execution (no tmux).
    For short/medium jobs where we can afford to block.
    """
    if payload.run_as_user:
        # escape single quotes for safe su -c
...
def build_job_script(job_id: str, payload: JobRequest) -> str:
    """
    Build a per-job shell script that:
      - runs the command (optionally via su - user)
      - redirects stdout/stderr to logs
      - writes exit code to 'exit'
      - writes 'finished' to 'status'

    This avoids crazy quoting inside tmux and is very robust.
    """
    ...
    # mark as running at creation
    write_file(status_file, "running")

    # Build base command
    if payload.run_as_user:
        # Carefully escape single quotes in the original command
        cmd_escaped = payload.command.replace("'", "'\"'\"'")
        run_line = (
    ...
    # IMPORTANT: 755 so tmux (or any user) can execute it
...
def execute_job(job_id: str, payload: JobRequest):
    """
    Asynchronous execution (with or without tmux).
    Uses per-job shell script + file-based tracking.

      - if use_tmux=True:
          /usr/bin/tmux new-session -d -s agent_<jobid> /path/to/run.sh
      - else:
    ...
    # We do NOT block here. /status/<job_id> will see when it finishes.
...
async def run_job(request: Request, payload: JobRequest):
    """
    Entry point used by all 3 operators.

    - sync=True,  use_tmux=False → synchronous (execute_sync)
    - sync=False, use_tmux=False → async non-tmux (execute_job)
    - sync=False, use_tmux=True  → async tmux (execute_job)
    """
    await validate_token(request)
